...
def main():
    ...
    for i, batch in tqdm(enumerate(dl_val), total=task2num_iters[task]):
        loss, score, batch_size, results, others, vil_prediction, target = EvaluatingModel(config, task_cfg, device, task, batch,
                                                                   model, dl_val, criterion, results, others)
        
        print(args.output_dir+'/'+task_cfg[task]['val_annotations_jsonpath'].split('/')[-1][:-6]+"-logits.txt")
        with open(args.output_dir+'/'+task_cfg[task]['val_annotations_jsonpath'].split('/')[-1][:-6]+"-logits.txt", "w") as f:
          for l in vil_prediction:
            for v in l:
              f.write(str(float(v))+',')
            f.write('\n')
